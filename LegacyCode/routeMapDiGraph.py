...
routeMap[71] = ['NE 85th St & 35th Ave NE', 'NE 65th St & 35th Ave NE', 'NE 65th St & 15th Ave NE', 'University Way NE & NE 45th St', 'Fairview Ave N & Denny Way',
				'Downtown Seattle Tunnel University Street Station Bay C', 'Downtown Seattle TunnelInternational District Station Bay C', '2nd Ave & Pike St']
...
routeMap[49] = ['12th Ave NE & NE 47th St', 'Estlake Ave E & Harvard Ave E', 'Broadway & E Roy St', 'Broadway & E Pine St', 'Pine St & 5th Ave']
...
for node in G.nodes():
	for node2 in G.nodes():
		if node != node2:
			x = set(routeMap[node]).intersection(set(routeMap[node2]))
			if x:

				G.add_edge(node, node2,weight=len(x))
...
#nx.draw_spring(G)
#nx.draw_networkx_edge_labels(G,pos,edge_labels={'1,2' : 1, '1,3' : 2},label_pos=0.5,font_size=12,font_color='k',font_family='sans-serif',font_weight='normal',alpha=1.0, bbox=None, ax = None, rotate=True)
#mpl.show()
for (u,v,d) in G.edges(data=True):
	print(str(u) + ', ' + str(v) + ' : ' + str(d['weight']))
#print(G.nodes())
#print(G.edges())
#for node in G.nodes():
#	print(str(node) + ' : ' + str(len(G.edges(node))))