...
for route in routeNumbers:
	if 'Downtown Seattle Tunnel International District Station Bay C' in routeMap[route]:
		usedRoutes.append(route)
		for stop in routeMap[route]:
			G.add_node(stop)

for route in usedRoutes:
	count=0
	while count < len(routeMap[route])-1:
		G.add_edge(routeMap[route][count], routeMap[route][count+1])
		count = count+1
